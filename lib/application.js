...
Application.prototype.themes = function() {
   var pathToThemes = this.paths.THEMES;
   return q.nfcall( fs.readdir, pathToThemes )
      .then( function( entries ) {
         return entries.filter( function( entry ) {
            return /\.theme$/.test( entry );
         } ).map( function( entry ) {
            return path.join( pathToThemes, entry );
         } );
      } );
};
