...
exports.create = function( log, config ) {
   ...
   try {
      ...
      return laxarTooling.artifactsCollector.create( log, requirejsHelper, config );
   }
   catch( err ) {
      log.error( 'Encountered error loading laxar-tooling:', err );
      log.error( 'Using fallback' );
   }
   return create( log, requirejsHelper, config );
};

function create( log, moduleLookup, config ) {
   'use strict';
   ...
   var requirejsHelper = moduleLookup;
