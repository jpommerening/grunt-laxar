...
describe( 'FlowTracer', function() {
   ...
   beforeEach( function() {
      ...
      flowTracer = FlowTracer.create( httpClient, pageLoader, 'my_widgets', 'my_layouts' );
   } );
   ...
   describe( '.pagesForFlow( flow )', function() {
      ...
      it( 'finds all pages reachable from the given flow', function( done ) {
         flowTracer.pagesForFlow( 'flow.json' )
            .then( function( pages ) {
   ...
   describe( '.widgetsForFlow( flow )', function() {
      ...
      it( 'finds all widgets in all pages reachable from the given flow', function( done ) {
         flowTracer.widgetsForFlow( 'flow.json' )
   ...
   describe( '.controlsForFlow( flow )', function() {
      ...
      it( 'finds all controls in all pages reachable from the given flow', function( done ) {
         flowTracer.controlsForFlow( 'flow.json' )
   ...
   describe( '.widgetsAndControlsForFlow( flow )', function() {
      ...
      it( 'finds all widgets and controls in all pages reachable from the given flow', function( done ) {
         flowTracer.widgetsAndControlsForFlow( 'flow.json' )
   ...
   describe( '.layoutsForFlow( flow )', function() {

      it( 'finds all layouts for all pages reachable from the given flow', function( done ) {
         flowTracer.layoutsForFlow( 'flow.json' )
            .then( function( layouts ) {
               expect( layouts ).toEqual( [
                  'my_layouts/someLayout',
                  'my_layouts/someOtherLayout'
               ] );
            } ).nodeify( done );
      } );
