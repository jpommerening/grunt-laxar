...
/* TODO:
 * - Fix mixed up argument order and adjust tasks,
 * - Remove gatherWidgetsAndControls
 **/
WidgetCollector.create = function create( httpClient, baseUrl, pageLoader ) {
   var collector = new WidgetCollector( httpClient, pageLoader, baseUrl );
   collector.gatherWidgetsAndControls = function( pathToWidgets, pathToFlow ) {
      collector.baseUrl_ = pathToWidgets;
      return collector.widgetsAndControlsForFlow( pathToFlow );
   };
   return collector;
};
...
WidgetCollector.prototype.controlsForFlow = function controlsForFlow( pathToFlow ) {
   ...
   return mapFlatUnique( promise, function( widgetName ) {
      return findControlsForWidget( httpClient, path.join( baseUrl, widgetName ) );
   } );
};
...
WidgetCollector.prototype.widgetsAndControlsForFlow = function widgetsAndControlsForFlow( pathToFlow ) {
   ...
   return q.spread( [
      ...
      return {
         widgets: widgets.map( widgetModule ),
         controls: controls
      };
   } );
};
